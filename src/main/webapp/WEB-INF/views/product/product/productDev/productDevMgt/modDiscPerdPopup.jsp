...
$(document).ready(function() {
	...
	$("#oprndRefFl1, #oprndFctrCd1, #oprndRefFl2, #oprndFctrCd2").selectmenu("enable");
	$("#oprnd1, #oprnd2").addClass('active'); 
	$("#oprnd1, #oprnd2").attr('enable', true);
	...
	if ($("#oprndRefFl1").val() == "0") {
		$("#oprndFctrCd1").selectmenu("disable");
		$("#oprnd1").removeClass('not-active'); 
		$("#oprnd1").removeAttr('disabled');
	} else if ($("#oprndRefFl1").val() == "1") {
		$("#oprndFctrCd1").selectmenu("enable");
		$("#oprnd1").addClass('not-active'); 
		$("#oprnd1").attr('disabled', true);
	} else {
	...
	if ($("#oprndRefFl2").val() == "0") {
		$("#oprndFctrCd2").selectmenu("disable");
		$("#oprnd2").removeClass('not-active'); 
		$("#oprnd2").removeAttr('disabled');
	} else if ($("#oprndRefFl2").val() == "1") {
		$("#oprndFctrCd2").selectmenu("enable");
		$("#oprnd2").addClass('not-active'); 
		$("#oprnd2").attr('disabled', true);
	} else {
...
/* 	
	...
	$('#modDiscPerdPopupBtnInsert').click(function() {
		var param = checkValidation();        
		if(param != false){
			
			$.ajax({
				url : 'modDiscPerd.json',
				type : 'POST',
				async: false,
				data : param,
				success : function(data) {
					...
					if(data.result != "0" && data.result != "-1"){
						...
						$("#modDiscPerdPopupBtnClose").trigger('click');
...
function checkValidation(){
	...
	param.rateItmCd = $("#modDiscPerdPopupRateItmCd").val();
	...
	<table class="writeview">
		...
		<thead>
	            ...
	            <td>
					...
					<option value=""><spring:message code="LAB.M15.LAB00002"/></option>
						<c:forEach items="${operator}" var="operator" varStatus="status">
							<option value="${operator.commonCd}" <c:if test="${discPerd.operator eq operator.commonCd}"> selected="selected" </c:if> >${operator.commonCdNm}</option>
						</c:forEach>
					</select>                                            
				</td>
			</tr>		
	            ...
	            <td>
					...
					<option value=""><spring:message code="LAB.M15.LAB00002"/></option>
						<c:forEach items="${oprndRefFl1}" var="oprndRefFl1" varStatus="status">
							<option value="${oprndRefFl1.commonCd}" <c:if test="${discPerd.oprndRefFl1 eq oprndRefFl1.commonCd}"> selected="selected" </c:if>>${oprndRefFl1.commonCdNm}</option>
						</c:forEach>
					</select>                                            
				</td>
			</tr>
	            ...
	            <td>
					...
					<option value=""><spring:message code="LAB.M15.LAB00002"/></option>
						<c:forEach items="${oprndFctrCd}" var="oprndFctrCd" varStatus="status">
							<option value="${oprndFctrCd.fctrCd}" <c:if test="${discPerd.oprndFctrCd1 eq oprndFctrCd.fctrCd}"> selected="selected" </c:if> >${oprndFctrCd.fctrNm}</option>
						</c:forEach>					
					</select>                                            
				</td>
			</tr>					
			<tr>
	            ...
	            <td>
					<input type="text" id="oprnd1" name="oprnd1" value="${discPerd.oprnd1}" class="w100p">                                            
				</td>
			</tr>	
	            ...
	            <td>
					...
					<option value=""><spring:message code="LAB.M15.LAB00002"/></option>
						<c:forEach items="${oprndRefFl2}" var="oprndRefFl2" varStatus="status">
							<option value="${oprndRefFl2.commonCd}" <c:if test="${discPerd.oprndRefFl2 eq oprndRefFl2.commonCd}"> selected="selected" </c:if> >${oprndRefFl2.commonCdNm}</option>
						</c:forEach>
					</select>                                            
				</td>
			</tr>
			<tr>
	            ...
	            <td>
					...
					<option value=""><spring:message code="LAB.M15.LAB00002"/></option>
						<c:forEach items="${oprndFctrCd}" var="oprndFctrCd" varStatus="status">
							<option value="${oprndFctrCd.fctrCd}" <c:if test="${discPerd.oprndFctrCd2 eq oprndFctrCd.fctrCd}"> selected="selected" </c:if> >${oprndFctrCd.fctrNm}</option>
						</c:forEach>					
					</select>                                            
				</td>
			</tr>					
			<tr>
	            ...
	            <td>
					<input type="text" id="oprnd2" name="oprnd2" value="${discPerd.oprnd2}" class="w100p">                                            
				</td>
			</tr>		
...
<!--// center -->
<div class="btn_box">
	<a class="blue-btn" href="#" id="modDiscPerdPopupBtnInsert"><span class="confirm_icon"><spring:message code="LAB.M09.LAB00048" /></span></a>
	<a class="grey-btn close" href="#" id="modDiscPerdPopupBtnClose"><span class="cancel_icon"><spring:message code="LAB.M03.LAB00027" /></span></a>
</div>

<input id="modDiscPerdPopupRateItmCd" type='text' value="${productDevMgt.rateItmCd}"  hidden />
