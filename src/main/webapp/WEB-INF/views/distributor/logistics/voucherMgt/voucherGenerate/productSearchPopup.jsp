			...
			<tr>
				...
				<td colspan="3">
					...
					<select class="w200" id="soIdSeP" name="soIdSeP">
						<c:if test="${productMngVO.soId != null && productMngVO.soId != ''}">
							<c:forEach items="${session_user.soAuthList}" var="soAuthList" varStatus="status">
								<option value="${soAuthList.so_id}" <c:if test="${soAuthList.so_id eq productMngVO.soId}"> selected="selected" </c:if> >${soAuthList.so_nm}</option>
							</c:forEach>
						</c:if>
						<c:if test="${productMngVO.soId == null || productMngVO.soId == ''}">
							<c:forEach items="${session_user.soAuthList}" var="soAuthList" varStatus="status">
