...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#btn_insert").click(function() {
			...
			var rowId  = $("#distributionUnitPriceTable").jqGrid("getGridParam", "selrow" );
			if(rowId == null){
				alert('<spring:message code="MSG.M03.MSG00030" />');		//등록할 제품을 선택해 주세요.
				return;
			}
			
			var url="dstrbUtPrcDtlInsertPopUp.ajax";
		...
		$("#btn_update").click(function() {
			...
			var rowId  = $("#distributionUnitPriceTable2").jqGrid("getGridParam", "selrow" );
			if(rowId == null){
				alert('<spring:message code="MSG.M07.MSG00095" />');	//수정할 단가내역을 선택해 주세요.
				return;
			}
			
			var url="dstrbUtPrcDtlUpdatePopUp.ajax";
			var param = new Object();
			$("#popup_dialog").css("width", 700);
	...
	function initGrid() {
		...
		$("#distributionUnitPriceTable").jqGrid({
			
		   	url:'distributionUnitPriceListAction.json?',
		   	postData : param,
		    mtype:"POST",
		   	datatype: "json",
		   	...
		   	pager: '#distributionUnitPricePager',
	        ...
	        loadComplete: function(obj){
	        	$("#distributionUnitPriceTable").setGridWidth($('#distributionUnitPriceGrid').width(),false); //그리드 테이블을 DIV(widht 100% : w100p)로 감싸서 처리
		    },
	...
	function searchData(){
		...
		$("#distributionUnitPriceTable").clearGridData();  // 이전 데이터 삭제
		var param = new Object();
		param.soId = $("#searchSoId").val();
        ...
        $("#distributionUnitPriceTable").setGridParam({ postData: param }).trigger("reloadGrid");
        
        $("#distributionUnitPriceTable2").clearGridData();  // 이전 데이터 삭제
	...
	function initGrid2() {
		...
		$("#distributionUnitPriceTable2").jqGrid({
			
		   	url:'dstrbUtPrcDtlListAction.json?',
		   	postData : param,
		    mtype:"POST",
		   	datatype: "json",
		   	...
		   	rowNum:10,					//한번에 노출되는 row 수
		   	rowList:[5,10,20,30,50],	//선택시 노출되는 row 수
		   	pager: '#distributionUnitPricePager2',
			sortable : true,			//드래그로 컬럼간의 위치 변경 가능 여부
		    viewrecords: true,	
			height: 180,				//화면 상태에 따라 크기 조절 할 것
			onCellSelect : function(rowId, index, contents, event){
		    ...
		    sortable: { update: function(permutation) {
		    	$("#distributionUnitPriceTable2").setGridWidth($('#distributionUnitPriceGrid2').width(),false); //그리드 테이블을 DIV(widht 100% : w100p)로 감싸서 처리
	    		}
	    	}
		});
	...
	function searchData2(rowId){
		...
		var data = $("#distributionUnitPriceTable").getRowData(rowId);
        ...
        $("#distributionUnitPriceTable2").setGridParam({ postData: param }).trigger("reloadGrid");
	...
	function deleteData(){
		...
		if(check){
			...
			var rowId  = $("#distributionUnitPriceTable2").jqGrid("getGridParam", "selrow" );
			var data = $("#distributionUnitPriceTable2").getRowData(rowId);
			
			param.prcDtlSeq = data.prcDtlSeq;
			
			
			$.ajax({
				url : 'deleteDstrbUtPrcDtl.json',
				type : 'POST',
				async: false,
				data : param,
...
<div id="distributionUnitPriceGrid">
	<table id="distributionUnitPriceTable" class="w100p"></table>
	<div id="distributionUnitPricePager"></div>
</div>
...
<div class="main_btn_box">
	...
	<div class="fr mt10">
		<div class="ask"><spring:message code="MSG.M15.MSG00003"/><!-- ※ 유통단가는 VAT제외 입니다.  --></div>
	</div>
</div>

<div id="distributionUnitPriceGrid2">
	<table id="distributionUnitPriceTable2" class="w100p"></table>
	<div id="distributionUnitPricePager2"></div>
</div>
