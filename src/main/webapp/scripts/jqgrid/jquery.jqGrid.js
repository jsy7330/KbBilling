...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			_index : {},
			grouping : false,
			groupingView : {groupField:[],groupOrder:[], groupText:[],groupColumnShow:[],groupSummary:[], showSummaryOnHide: false, sortitems:[], sortnames:[], summary:[],summaryval:[], plusicon: 'ui-icon-circlesmall-plus', minusicon: 'ui-icon-circlesmall-minus', displayField: [], groupSummaryPos:[], formatDisplayField : [], _locgr : false, displayHeader : false},
			ignoreCase : false,
			cmTemplate : {},
			idPrefix : "",
			multiSort :  false
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select id='"+pgcnt+"_pageSelect'  class='ui-pg-selbox' role='listbox'>";
			...
			*/
			...
			$("#"+pgcnt + "_pageSelect").selectmenu();
			$("#"+pgcnt + "_pageSelect").next('.ui-selectmenu-button').css('border','1px solid #eaeaea');
		    ...
		    $("#"+pgcnt + "_pageSelect").selectmenu({
		        change: function() {
					...
					ts.p.page = Math.round(ts.p.rowNum*(ts.p.page-1)/this.value-0.5)+1;
					ts.p.rowNum = this.value;
					if(ts.p.pager) { $("#"+pgcnt + "_pageSelect").val(this.value); $("#"+pgcnt + "_pageSelect").selectmenu("refresh");}
					if(ts.p.toppager) { $("#"+pgcnt + "_pageSelect").val(this.value);  $("#"+pgcnt + "_pageSelect").selectmenu("refresh");}
					populate();
					return false;
		        }
		    });
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key === 13) {
					...
					ts.p.page = ($(this).val()>0) ? ((ts.p.lastpage < $(this).val()) ? ts.p.lastpage : $(this).val()) : ts.p.page;
					populate();
					return false;
				}
				return this;
		...
		var	tfoot = "";
		if(ts.p.footerrow) { tfoot += "<table role='grid' style='width:"+ts.p.tblwidth+"px' class='ui-jqgrid-ftable w100p' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+dir+"'>"; }
		...
		$("th",thr).each(function ( j ) {
			...
			if(ts.p.footerrow) { tfoot += "<td role='gridcell' class='ui-state-default'"+formatCol(j,0,'', null, '', false)+">&#160;</td>"; }
		...
		firstr = null;
		var hTable = $("<table class='ui-jqgrid-htable w100p' style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
		...
		return this.each(function(){
			...
			if($t.p.tblwidth) {
				...
				if($($t.grid.bDiv)[0].clientHeight < $($t.grid.bDiv)[0].scrollHeight){
					$('table:first',$t.grid.hDiv).css("padding-right",scw + "px");
				}else{
					$('table:first',$t.grid.hDiv).css("padding-right","0px");
				}
				...
				if($t.p.footerrow) {
					...
					if($($t.grid.bDiv)[0].clientHeight < $($t.grid.bDiv)[0].scrollHeight){
						$('table:first',$t.grid.sDiv).css("padding-right",scw + "px");
					}else{
						$('table:first',$t.grid.sDiv).css("padding-right","0px");
					}
		...
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				if(grp.groupSummaryPos[n.idx] === 'header')  {
					str += "<tr id=\""+hid+"\"" +((grp.groupCollapse && n.idx>0) || grp.displayHeader == false ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\">"+icon+$.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary)+"</td>";
					str += buildSummaryTd(i, n.idx-1, grp.groups, 1);
					str += "</tr>";
				} else {
					var colspanSize = colspans - hiddenCnt;
					str += "<tr id=\""+hid+"\"" +((grp.groupCollapse && n.idx>0) || grp.displayHeader == false ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+colspanSize+"\">"+icon+$.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary)+"</td></tr>";
				}
