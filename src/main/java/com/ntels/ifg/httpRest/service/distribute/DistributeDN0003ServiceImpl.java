...
@Service
public class DistributeDN0003ServiceImpl implements InterfaceProcessService {
	...
	private static final String OP_CODE = "DN0003";
	...
	public ResponseVO processInterface(String svcCode, String opCode, RequestVO request) {
		...
		try{
			...
			for(Map<String,String> parameter :list){
				...
				String org_id = distributeMapper.getDealerOrgId(parameter.get("org_id"));
				
				if("C".equals(itemTyp)){
					
					
					//단말기 이전 상태를 조회
					Map<String,String> status = distributeMapper.getDtItemSale(parameter.get("item_id"),parameter.get("eqt_seq"),org_id);
					...
					if(status == null){
						...
						parameter.put("org_id", org_id);
						parameter.put("seq", sequenceService.createNewSequence(Consts.SEQ_CODE.DT_HIST_SEQ, "1", 10));
						
						distributeMapper.updateProc(parameter, DateUtil.sysdate(),DateUtil.getDateStringYYYYMMDDHH24MISS(0));
						distributeMapper.insertProcHist(parameter, DateUtil.sysdate(),DateUtil.getDateStringYYYYMMDDHH24MISS(0));
					...
					//USIM 이전 상태 조회
					Map<String,String> status = distributeMapper.getUsimItemSale(parameter.get("item_id"),parameter.get("eqt_seq"),org_id);
					//이전 상태가 NULL이면 Null
					if(status == null){
						result.put("RESULT", "U");
					}else{
						...
						parameter.put("org_id", org_id);
						parameter.put("seq", sequenceService.createNewSequence(Consts.SEQ_CODE.DT_HIST_SEQ, "1", 10));
						distributeMapper.updateUsimProc(parameter, DateUtil.sysdate(),DateUtil.getDateStringYYYYMMDDHH24MISS(0));
						distributeMapper.insertUsimProcHist(parameter, DateUtil.sysdate(),DateUtil.getDateStringYYYYMMDDHH24MISS(0));
