...
/**
 * <PRE>
 * 1. ClassName: OrderProcessI104ServiceImpl
 * 2. FileName : OrderProcessI104ServiceImpl.java
 ...
 */
@Service
public class OrderProcessI104ServiceImpl extends OrderProcessServiceImpl implements OrderProcessService{
    private final Log logger = LogFactory.getLog(getClass());

    private static final String[] SVC_CD = {"SV096"}; //IOT 서비스
	private static final String ORDER_TP = "104"; //오더유형
	private static final String[] CTRT_STAT = {"50","90"}; //이전 계약 상태
	...
	public OrderRequestInfoVO createOrder(OrderRequestInfoVO orderInfo)
		...
		logger.debug("########## IOT 104 Create Order START ###########");
		...
		logger.debug("########## IOT 104 Create Order End ###########");
	...
	public OrderRequestInfoVO cancelOrder(OrderRequestInfoVO orderInfo)
		...
		logger.debug("########## IOT 104 Cancel Order START ###########");
		...
		logger.debug("########## IOT 104 Cancel Order END ###########");
	...
	public OrderRequestInfoVO saveOrder(OrderRequestInfoVO orderInfo)
		...
		logger.debug("########## IOT 104 Save Order START ###########");
		...
		logger.debug("########## IOT 104 Save Order END ###########");
		return orderInfo;
	}
	...
	public OrderRequestInfoVO progressOrder(OrderRequestInfoVO orderInfo) throws ServiceException {
		...
		logger.debug("########## IOT 104 Progress Order START ###########");
		...
		logger.debug("########## IOT 104 Progress Order END ###########");
		return orderInfo;
	}
	...
	public void processPreOrderMake(OrderRequestInfoVO orderInfo) throws ServiceException {
		...
		if(StringUtils.isEmpty(orderInfo.getTermHopeDt())){
			String[] arg = {MessageUtil.getMessage("LAB.M14.LAB00079")}; // 해지 취소 요청일
			throw new ServiceException("MSG.M13.MSG00027", arg); // 필수값 체크
		}
		
		//해지 취소 요청일은 당일만 가능
		if(!orderInfo.getOrderReqDt().equals(orderInfo.getTermHopeDt())){ // 해지 취소 요청일은 변경 불가
