...
$(document).ready(function() {
	...
	popupPageInit();
  	...
  	$("#pause").click(function () {	//정지해제 radio 버튼 클릭 막기
		return false;
	});
	...
	$("#popupProdInfoTabs li").click(function () {
  	...
  	//비고 keyup 이벤트
  	$('#popupChgPauseInfo_chngDetailResn').keyup(function(){
  		var str = getMaxStr($('#popupChgPauseInfo_chngDetailResn').val(), 300);
  		if(str != $('#popupChgPauseInfo_chngDetailResn').val()){
  			$('#popupChgPauseInfo_chngDetailResn').val(str);
  		}
  	});
...
function getCodeList(){
	//일시정지 코드 리스트 조회
	var url = '/customer/contract/contract/orderManagement/getSuspentionReleaseCodeListAction.json';
	$.ajax({
       url:url,
       type:'POST',
       data : {
...
function getPeriodInfo(){
	var url = '/customer/contract/contract/orderManagement/getSuspendedPeriodAction.json';
	$.ajax({
        url:url,
        type:'POST',
        data : {
...
function getOrderInfoInProgress(){
	...
	var url = '/customer/contract/contract/orderManagement/getOrderInfoSuspentionReleaseInProgressAction.json';
	$.ajax({
        url:url,
        type:'POST',
        data : {
...
function popupPageInit(){
	...
	//$('#btn_cal2').click(function(e){e.preventDefault();$(this).prev().focus();});
...
function orderPopup_setDisable(){
	...
	$("#popupChgPauseInfo_chngDetailResn").addClass('not-active');
	$("#popupChgPauseInfo_chngDetailResn").attr('disabled',true);
}
</script>
...
<!-- //popupProdInfo_onetimeChargeInfoDiv -->
		...
		<div class='table_col_box bdT h150'>
			<div class='col_left'>
				...
				<div id='popChgSuspentionInfo_suspentionListDiv' class='col_box_table'>
				...
				<table class='writeview'>
					<tbody>
						<tr class='col1'>
							...
							<td>
								<input type='radio' id='pause' name='TemporaryStop' required='required'/>
								<label for='pause'><spring:message code="LAB.M08.LAB00142"/></label>
								<input type='radio' id='clearPause' name='TemporaryStop' required='required'  checked='checked'/>
								<label for='clearPause'><spring:message code="LAB.M09.LAB00223"/></label>
							</td>
						</tr>
						<tr class='col1'>
							...
							<td>
								<div class="date_box">
									<div class="inp_date w45p">
							 			<input  type="text" id="popupChgPauseInfo_startHopeDt" name="popupChgPauseInfo_startHopeDt" value="<fmt:formatDate value="<%= new java.util.Date()%>"  pattern="${dateToStrFormat4}"/>" class="datepicker" readonly="readonly"><a href="#" id="btn_cal1" class="btn_cal"></a> 
									</div>
									<span class="dash">~</span>
									<div class="inp_date w45p">
									  <input  type="text"  id="popupChgPauseInfo_endHopeDt" name="popupChgPauseInfo_endHopeDt" value="<fmt:formatDate value="<%= new java.util.Date()%>" pattern="${dateToStrFormat4}"/>" class="datepicker" readonly="readonly"><a href="#" id="btn_cal2" class="btn_cal"></a>
									</div>
								</div>
							</td>
						</tr>
