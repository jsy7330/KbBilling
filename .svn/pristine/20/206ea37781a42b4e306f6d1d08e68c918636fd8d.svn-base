...
$(document).ready(function() {
	...
	//권한정보 GRID
	$("#soAuthGrid").jqGrid({
		url : '/system/auth/authMng/soAuthMng/soAuthListAction.json',
		datatype : 'json',
		mtype: 'POST',
		postData : {
		},
		colModel: [ 
		     	{ label: '<spring:message code="LAB.M07.LAB00010"/>', name: 'soId', width : 100},
			    { label: '<spring:message code="LAB.M07.LAB00013"/>', name: 'soNm', width : 150}
	...
	$("#soAuthGrid").setGridWidth($('#soAuthDiv').width(), false); //그리드 테이블을 DIV(widht 100% : w100p)로 감싸서 처리
	...
	$( "#condUserNm" ).keypress(function(event) {
	 ...
	 $('#condOrgNm_popup').on('click',function (e) {	//검색조건 조직검색 이벤트
	...
	$("#updateBtn").click(function() {	//추가
		...
		var data = $("#soAuthGrid").jqGrid('getDataIDs');
	 
		var tmpSoId=$("#tmpSoId").val();
		 
		for(var i=0;i<data.length;i++){
			var data2 = $("#soAuthGrid").getRowData(data[i]);
			tmpSoId+=data2.soId+",";
		}
		tmpSoId=tmpSoId.slice(0,-1);
	
		$("#tmpSoId").val(tmpSoId);
	 	fnNew(); 
	 
		$("#soAuthGrid").setGridParam({
 	        postData : {
 	        	condUserId : $("#condUserId").val(),
 				condUserNm : $("#condUserNm").val(),
 				condOrgId : $("#condOrgId").val(),
...
function clickId(id){	//사용자에 해당하는 권한 그룹보여주기
	...
	var soAuth = 'userId=' + $('#idUser').val();
	
	$("#soAuthGrid").clearGridData();
	
	
	$.ajax({
		url:'/system/auth/authMng/soAuthMng/getBaseSoIdAction.json',
		type:'POST',
		data : soAuth,
		dataType: 'json',
		success: function(data){
	...
	$("#soAuthGrid").trigger("reloadGrid"); 
	$("#soAuthGrid").setGridWidth($('#userPerAuthDiv').width(), false); 
}
...
function fnNew(){
	//기본그룹 팝업 
    	...
    	var url="/system/common/common/soSearchMng/soSearchPopup.ajax";
    	var param = new Object();
    	
    	param.soId = $("#tmpSoId").val();	//리턴받는 orgId 의 태그아이디값
...
function insertSoAuthInfo(soId,userId){
	var soAuth = 'userId=' +userId;
	 soAuth = soAuth + '&soId=' + soId;
	 $.ajax({
          url:'/system/auth/authMng/soAuthMng/insertAction.json',	
          type:'POST',
          data : soAuth,
          dataType: 'json',
          success: function(data){
...
function fnDelete(){	//체크한것 삭제
	var rowid=  $("#soAuthGrid").jqGrid('getGridParam', 'selarrrow');
	var tmpId="";
	if(rowid.length==0){
	...
	if(confirm('<spring:message code="MSG.M07.MSG00054"/>')==true){
		...
		for(var i=0; i<rowid.length; i++){
			var rowObject = $("#soAuthGrid").getRowData(rowid[i]);				
			if(rowObject.soId==$("#baseId").val()){
				alert(rowObject.soNm+'<spring:message code="MSG.M02.MSG00011"/>');
				return;
			}
		...
		var soAuth = 'userId=' + $('#idUser').val();
		soAuth = soAuth + '&soId=' + tmpId;
	
		$.ajax({
			url:'/system/auth/authMng/soAuthMng/deleteAction.json',
			type:'POST',
			data : soAuth,
			dataType: 'json',
			success: function(data){
...
<input type="hidden" id="tmpSoId" name="tmpSoId"/>
...
<div class="group_box mt10">
		...
		<table class="writeview">
			...
			<thead>
				<tr>
					...
					<td><input id="condUserId" name="condUserId" type="text" class="w180" /></td>
					<th><spring:message code="LAB.M07.LAB00071"/></th>
					<td><input id="condUserNm" name="condUserNm" type="text" class="w180" /></td>
				</tr>
				<tr>
	...
	<div class="user_right">
		<div class="main_btn_box">
			<div class="fl">
				<h4 class="sub_title"><spring:message code="LAB.M07.LAB00007"/></h4>
			</div>
		</div>
		<div id='soAuthDiv' class="w100p">
			<table id="soAuthGrid" class="w100p"></table>
		</div>
	</div>
