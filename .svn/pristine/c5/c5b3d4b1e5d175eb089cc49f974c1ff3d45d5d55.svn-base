...
$(document).ready(function() {
	...
	$("#serviceMgtSvcRateItmTypListBtnInsert").click(function() {
		insertData();            
	});	
	
	$("#serviceMgtSvcRateItmTypListSoId, #serviceMgtSvcRateItmTypListRateItmTyp, #rateLocat, #invItm").selectmenu()
		.selectmenu( "option", "width", "100%" );
	$("#serviceMgtSvcRateItmTypListSoId").selectmenu("disable");
	
	$("#chrgCtgry").addClass('not-active');
	$("#chrgCtgry").attr('disabled', true);
	
	$(document).on("keyup", "input:text[numberOnly]", function() {$(this).val( $(this).val().replace(/[^0-9]/gi,"") );});
	
	$("#serviceMgtSvcRateItmTypListRateItmTyp").selectmenu({
		change: function( event, ui ) {
			var tmpParam = new Object();
			var tmpSvcCd = $("#serviceMgtSvcRateItmTypListInsertSvcCd").val();
			var tmpRateItmTypCd = $(this).val();
			tmpParam.svcCd = tmpSvcCd;
			tmpParam.rateItmTypCd = tmpRateItmTypCd;
			
			$.ajax({
				url : 'getSaleItmComboList.json',
				type : 'POST',
				async: false,
				data : tmpParam,
				success : function(data) {
					
					$('#saleItm').find('option').remove(); 

					for(var i=0;i<data.serviceMgtSaleItmComboList.length;i++){
						$('#saleItm').append('<option value="'+data.serviceMgtSaleItmComboList[i].saleItmCd+'">'+data.serviceMgtSaleItmComboList[i].saleItmNm+'</option>');
					}
...
function chgInput() {
	$("#dispPriNo").val($("#serviceMgtSvcRateItmTypListRateItmTyp option:selected").attr("dispNo"));
	$("#chrgCtgry").val($("#serviceMgtSvcRateItmTypListRateItmTyp option:selected").attr("chrgCtgryNm"));
	$("#serviceMgtSvcRateItmTypListInsertChrgCtgry").val($("#serviceMgtSvcRateItmTypListRateItmTyp option:selected").attr("chrgCtgry"));
}
...
function checkValidation(){
	...
	var soId = $.trim( $("#serviceMgtSvcRateItmTypListSoId").val() );	//사업
	if(soId == ''){
		alert('<spring:message code="LAB.M07.LAB00003" /> <spring:message code="MSG.M08.MSG00042" />');	//사업을 선택하세요
		return false;
	}	
	
	var rateItmTyp = $.trim( $("#serviceMgtSvcRateItmTypListRateItmTyp").val() );	//요금유형
	if(rateItmTyp == ''){
		alert('<spring:message code="LAB.M08.LAB00052" /> <spring:message code="MSG.M08.MSG00042" />');	
		return false;
	}
	...
	var svcCd = $("#serviceMgtSvcRateItmTypListInsertSvcCd").val();
	var svcRateItmTypNm = $("#svcRateItmTypNm").val();
	var mainSvcCd = $("#serviceMgtSvcRateItmTypListInsertMainSvcCd").val();
	var chrgCtgry = $("#serviceMgtSvcRateItmTypListInsertChrgCtgry").val();
...
<div class="layer_box">
	<div class="ly_btn_box">
		<div class="fl">
			<h4 class="ly_title"><spring:message code="LAB.M07.LAB00176"/><!-- 서비스과금항목유형등록 --></h4>
		</div>
	</div>
	...
	<table class="writeview">
		...
		<thead>
			<tr>
	            ...
	            <td colspan="3">
					<select class="w100p" id="serviceMgtSvcRateItmTypListSoId" name="serviceMgtSvcRateItmTypListSoId">
			...
			<tr>
				...
				<td colspan="3">
					<select id=serviceMgtSvcRateItmTypListRateItmTyp name="serviceMgtSvcRateItmTypListRateItmTyp" class="w100p">
				        <c:forEach items="${rateItmTyp}" var="rateItmTyp" varStatus="status">
							<option value="${rateItmTyp.rateItmTypCd}" dispNo="${rateItmTyp.nextDispPriNo}" chrgCtgryNm="${rateItmTyp.chrgCtgryNm}" chrgCtgry="${rateItmTyp.chrgCtgry}" >${rateItmTyp.rateItmTypNm}</option>
						</c:forEach>
					</select>                                                                                   
				</td>	
			</tr>		
			<tr>		
				...
				<td colspan="3">
	       			<input type="text" id="svcRateItmTypNm" name="svcRateItmTypNm" class="w100p">                                            
	     		</td>
	 		</tr>
			<tr>		
				<th>
				...
				<td colspan="3">
	       			<input type="text" id="svcRateItmTypAbbrNm" name="svcRateItmTypAbbrNm" class="w100p">                                            
	     		</td>
	 		</tr>
			<tr>		
				<th>
				...
				<td colspan="3">
	       			<input type="text" id="chrgCtgry" name="chrgCtgry" class="w100p">                                            
	     		</td>
	 		</tr>		 			 		
			<tr>
				<th>
				...
				<td colspan="3">
					<select id=invItm name="invItm" class="w100p">
				        <c:forEach items="${invItm}" var="invItm" varStatus="status">
							<option value="${invItm.invItmCd}">${invItm.invItmNm}</option>
						</c:forEach>
					</select>                                                                                   
				</td>	
			</tr>
			<tr>		
				<th>
					<spring:message code="LAB.M10.LAB00081"/><!-- 출력우선순위 -->
				</th>
				<td colspan="3">
	       			<input type="text" id="dispPriNo" name="dispPriNo" class="w200" numberonly="true">                                            
	     		</td>
	 		</tr>			
			<tr>		
				<th>
				...
				<td colspan="3">
	       			<input type="text" id="ratePriNo" name="dispPriNo" class="w200" numberonly="true">                                            
	     		</td>
	 		</tr>		
			<tr>		
				<th>
				...
				<td colspan="3">
	       			<textarea id="svcRateItmTypDesc" name="svcRateItmTypDesc" class="w100p h100"></textarea>                                           
	     		</td>
	 		</tr>
			<tr>	
				<th>
...
<!--// center -->
<div class="btn_box">
	<a class="blue-btn" href="#" id="serviceMgtSvcRateItmTypListBtnInsert"><span class="confirm_icon"><spring:message code="LAB.M09.LAB00048" /></span></a>
	<a class="grey-btn close" href="#" id="btnClose"><span class="cancel_icon"><spring:message code="LAB.M03.LAB00027" /></span></a>
</div>

<input id="serviceMgtSvcRateItmTypListInsertSvcCd" type='text' value="${svcCd}"  hidden />
<input id="serviceMgtSvcRateItmTypListInsertMainSvcCd" type='text' value="${serviceMgt.targetMainSvcCd}"  hidden />
<input id="serviceMgtSvcRateItmTypListInsertChrgCtgry" type='text' value=""  hidden />
